...
(function ($, Drupal, CKEDITOR) {
  ...
  CKEDITOR.plugins.add('linkitextended', {
    icons: 'linkitextended',

    init: function (editor) {

      // Add the commands for link and unlink.
      editor.addCommand('linkitextended_command', {
        ...
        exec: function (editor) {
          ...
          var saveCallback = function (returnValues) {
            ...
            if (!linkElement && returnValues.attributes.href) {
              ...
              if (range.collapsed) {
                ...
                var text = new CKEDITOR.dom.text(returnValues.autotitle, editor.document);
          ...
          var dialogSettings = {
            title: linkElement ? editor.config.linkit_dialogTitleEdit : editor.config.linkit_dialogTitleAdd,
            dialogClass: 'editor-linkit-dialog'
          };

          // Open the dialog for the edit form.
          Drupal.ckeditor.openDialog(editor, Drupal.url('stasco_ckeditor_plugins/dialog/linkitextended/' + editor.config.drupal.format), existingValues, saveCallback, dialogSettings);
        }
      });

      editor.ui.addButton('LinkitExtended', {
        label: Drupal.t('Entity Links'),
        command: 'linkitextended_command'
      });
      ...
      editor.on('doubleclick', function (evt) {
        ...
        if (!element.isReadOnly()) {
          if (element.is('a')) {
            ...
            editor.getCommand('linkitextended_command').exec();
          }
        }
      });
